...
export class AuthController {
  ...
  async login(@Body() body: unknown, @Headers('x-request-id') requestId: string | undefined, @Headers('user-agent') userAgent: string | undefined, @Res({ passthrough: true }) response: Response) {
  ...
  @Post('mfa/challenge')
  @HttpCode(HttpStatus.OK)
  async challenge(@Body() body: unknown, @Headers('x-request-id') requestId: string | undefined, @Headers('user-agent') userAgent: string | undefined, @Res({ passthrough: true }) response: Response) {
  ...
  @Post('refresh')
  @HttpCode(HttpStatus.OK)
  async refresh(@Headers('x-request-id') requestId: string | undefined, @Headers('user-agent') userAgent: string | undefined, @Res({ passthrough: true }) response: Response) {
    const refreshToken = this.readRefreshCookie(response);
    if (!refreshToken) throw new BadRequestException('Refresh token cookie is required');
  ...
  @Post('logout')
  @HttpCode(HttpStatus.NO_CONTENT)
  async logout(@Headers('x-request-id') requestId: string | undefined, @Headers('user-agent') userAgent: string | undefined, @Res({ passthrough: true }) response: Response): Promise<void> {
    await this.authService.logout(this.readRefreshCookie(response), this.metadata(response, requestId, userAgent));
  ...
  private metadata(response: Response, requestId: string | undefined, userAgent: string | undefined) {
    ...
    const clientAddress = typeof forwardedFor === 'string' ? forwardedFor.split(',')[0]?.trim() : remoteAddress;
    const ipHash = clientAddress ? createHash('sha256').update(clientAddress, 'utf8').digest('hex') : undefined;
    return { requestId: requestId ?? 'unknown', ...(userAgent ? { userAgent } : {}), ...(ipHash ? { ipHash } : {}) };
  }
  ...
  private readRefreshCookie(response: Response): string | undefined {
    ...
    for (const part of cookieHeader.split(';')) {
      ...
      if (part.slice(0, separator).trim() !== REFRESH_TOKEN_COOKIE) continue;
  ...
  private setAuthCookies(response: Response, accessToken: string, refreshToken: string): void {
    response.cookie(ACCESS_TOKEN_COOKIE, accessToken, { ...COOKIE_OPTIONS, maxAge: ACCESS_COOKIE_MAX_AGE, path: '/' });
    response.cookie(REFRESH_TOKEN_COOKIE, refreshToken, { ...COOKIE_OPTIONS, maxAge: REFRESH_COOKIE_MAX_AGE, path: '/api/v1/auth' });
  }

  private clearAuthCookies(response: Response): void {
    response.clearCookie(ACCESS_TOKEN_COOKIE, { ...COOKIE_OPTIONS, path: '/' });
    response.clearCookie(REFRESH_TOKEN_COOKIE, { ...COOKIE_OPTIONS, path: '/api/v1/auth' });
  }
}
